...
type ClassId = 'swordfighter' | 'archer' | 'magician' | 'support';

type Rarity = 'common' | 'rare' | 'epic' | 'legendary';

type StatKey = keyof ClassStats;
...
const classData: Record<ClassId, ClassData> = {
  swordfighter: {
    id: 'swordfighter',
    name: 'Swordfighter',
...
const mapData: MapData[] = [
  {
    ...
    background:
      'linear-gradient(180deg, rgba(58, 184, 116, 0.92) 0%, rgba(22, 61, 39, 0.92) 100%), url(https://public-frontend-cos.metadl.com/mgx/img/nitro-legacy/map-forest.png)',
    accent: '#61ffb3',
  ...
  },
  {
    ...
    background:
      'linear-gradient(180deg, rgba(255, 154, 82, 0.9) 0%, rgba(115, 33, 15, 0.92) 100%), url(https://public-frontend-cos.metadl.com/mgx/img/nitro-legacy/map-canyon.png)',
    accent: '#ff7f50',
  ...
  },
  {
    ...
    background:
      'linear-gradient(180deg, rgba(120, 196, 255, 0.88) 0%, rgba(27, 65, 110, 0.9) 100%), url(https://public-frontend-cos.metadl.com/mgx/img/nitro-legacy/map-glacier.png)',
    accent: '#71d4ff',
    favored: ['magician']
  },
  {
    ...
    background:
      'linear-gradient(180deg, rgba(226, 184, 120, 0.9) 0%, rgba(92, 61, 45, 0.92) 100%), url(https://public-frontend-cos.metadl.com/mgx/img/nitro-legacy/map-haven.png)',
    accent: '#ffc76f',
    favored: ['support', 'magician']
  }
];

const weaponCatalog: Record<ClassId, WeaponEntry[]> = {
  swordfighter: [
...
const heroImage = document.querySelector<HTMLImageElement>('.pixel-hero');
const pixelPreview = document.querySelector<HTMLElement>('.pixel-preview');
...
const renderStats = (classInfo: ClassData) => {
  ...
  (Object.keys(statLabels) as StatKey[]).forEach((key) => {
...
const renderInventory = (activeClass: ClassId) => {
...
const renderWeaponColumns = () => {
  if (!weaponColumns) return;
  weaponColumns.innerHTML = '';
  weaponColumnRefs.clear();

  (Object.keys(weaponCatalog) as ClassId[]).forEach((classId) => {
...
const highlightMaps = (activeClass: ClassId) => {
...
const setActiveClassCard = (classId: ClassId) => {
  if (!classSelector) return;
  const cards = classSelector.querySelectorAll<HTMLElement>('.class-card');
  cards.forEach((card) => {
...
const setHeroVisuals = (classInfo: ClassData) => {
  if (heroImage) {
